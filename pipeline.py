...
# Shared validation
# --------------------------------------------------------------------------- #
async def _first_validating(cands: list[PDFCandidate], client, ua, timeout, browser_ua=None, limit=5):
...
async def _tier1(company, *, provider, crawler, client, store, blocklist, cfg) -> bool:
    ...
    good, verified = await _first_validating(crawl_pdf_cands, client, ua, timeout, browser_ua)
    if good:
