...
<div class="dashboard-container">
    ...
    <section class="table-section" id="quality-section" style="margin-top:var(--gap);">
        <h3>PDF/rule extraction quality <span style="color:var(--text-secondary);font-weight:400;" id="quality-summary"></span></h3>
        <div id="quality-bands" style="margin-bottom:12px;"></div>
        <div id="quality-flags" style="margin-bottom:12px;color:var(--text-secondary);font-size:0.9em;"></div>
        <table class="data-table">
            <thead><tr>
                <th>Ticker</th><th>Year</th><th>Statement</th><th>Confidence</th><th>Status</th><th>Flags</th>
            </tr></thead>
...
// ----- PDF/rule extraction quality section -----
(function renderQuality() {
    const q = DATA.quality;
    const sec = document.getElementById('quality-section');
    if (!q || !q.available) { if (sec) sec.style.display = 'none'; return; }
    document.getElementById('quality-summary').textContent =
        `${q.total} statement(s), mean confidence ${q.meanConfidence.toFixed(2)}`;
    document.getElementById('quality-bands').innerHTML = Object.entries(q.bands)
        .map(([k, v]) => `<span style="margin-right:16px;"><b>${v}</b> ${k}</span>`).join('');
    document.getElementById('quality-flags').innerHTML = 'Flags: ' +
        (Object.entries(q.flags).map(([k, v]) => `${k} (${v})`).join(' · ') || 'none');
    document.getElementById('quality-body').innerHTML = q.lowConfidence.map(r =>
        `<tr><td>${r.ticker}</td><td>${r.fiscalYear}</td><td>${r.statementType}</td>` +
        `<td>${r.confidence.toFixed(2)}</td><td>${r.status}</td>` +
        `<td style="color:var(--text-secondary);font-size:0.85em;">${r.reason || ''}</td></tr>`
    ).join('') || '<tr><td colspan="6" style="color:var(--text-secondary);">No low-confidence statements.</td></tr>';
})();
</script>
</body>
</html>
...
def build_quality(rules_db: str) -> dict:
    """Rule-extraction quality rollup from the separate rules DB (confidence
    distribution, flag frequency, lowest-confidence statements). {available:False}
    if the DB / confidence column isn't there yet."""
    from pathlib import Path as _P
    if not _P(rules_db).exists():
    ...
    try:
        try:
            rows = conn.execute(
                ...
                "FROM pdf_llm_status WHERE confidence IS NOT NULL").fetchall()
    ...
    return {"available": True, "total": len(rows),
            "meanConfidence": total_conf / len(rows), "bands": bands,
            "flags": dict(sorted(flags.items(), key=lambda kv: -kv[1])[:12]),
...
def main() -> None:
    ...
    ap.add_argument("--rules-db", default="output/pdf_financials.db")
    ...
    try:
        ...
        conn.close()
    data["quality"] = build_quality(args.rules_db)
