...
def build_data(conn: sqlite3.Connection) -> dict:
    ...
    # Fiscal-year coverage: how many companies have a statement for each
    # year, last 6 years only (older years thin out to single digits and just
    # clutter the chart -- see schema comment, coverage decays fast before 2020).
    year_coverage_rows = conn.execute(
        "SELECT fiscal_year, COUNT(DISTINCT ticker) FROM company_years "
        "GROUP BY fiscal_year ORDER BY fiscal_year DESC LIMIT 6"
    ).fetchall()
...
<div class="dashboard-container">
    <header class="dashboard-header">
        <div>
            ...
            <div class="subtitle">Structured 5-year financials from QuoteMedia (TSX/TSXV/CSE/XCNQ/NEOE)</div>
        </div>
        <div class="filters">
    ...
    <footer class="dashboard-footer">Click any company row to expand its 5-year key-metric grid. Generated by build_dashboard.py from financials.db.</footer>
</div>
